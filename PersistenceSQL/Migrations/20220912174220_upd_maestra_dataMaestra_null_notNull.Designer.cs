// <auto-generated />
using System;
using ERP.PersistenceSQL;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

namespace ERP.PersistenceSQL.Migrations
{
    [DbContext(typeof(SqlERPDbContext))]
    [Migration("20220912174220_upd_maestra_dataMaestra_null_notNull")]
    partial class upd_maestra_dataMaestra_null_notNull
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("ProductVersion", "5.0.14")
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("ERP.Domain.Entities.Maestras.DataMaestra", b =>
                {
                    b.Property<string>("nmdato")
                        .HasColumnType("VARCHAR(150)");

                    b.Property<string>("Maestranmmaestro")
                        .HasColumnType("VARCHAR(50)");

                    b.Property<string>("cddato")
                        .HasColumnType("VARCHAR(20)");

                    b.Property<string>("cddato1")
                        .HasColumnType("VARCHAR(100)");

                    b.Property<string>("cddato2")
                        .HasColumnType("VARCHAR(100)");

                    b.Property<string>("cddato3")
                        .HasColumnType("VARCHAR(100)");

                    b.Property<string>("dsdato")
                        .HasColumnType("VARCHAR(100)");

                    b.Property<DateTime?>("febaja")
                        .HasColumnType("DATETIME");

                    b.Property<DateTime?>("feregistro")
                        .HasColumnType("DATETIME");

                    b.Property<string>("nmmaestro")
                        .IsRequired()
                        .HasColumnType("VARCHAR(50)");

                    b.HasKey("nmdato");

                    b.HasIndex("Maestranmmaestro");

                    b.ToTable("TblDataMaestra", "Maestra");
                });

            modelBuilder.Entity("ERP.Domain.Entities.Maestras.Maestra", b =>
                {
                    b.Property<string>("nmmaestro")
                        .HasColumnType("VARCHAR(50)");

                    b.Property<string>("cdmaestro")
                        .HasColumnType("VARCHAR(5)");

                    b.Property<string>("dsmaestro")
                        .HasColumnType("VARCHAR(100)");

                    b.Property<DateTime?>("febaja")
                        .HasColumnType("DATETIME");

                    b.Property<DateTime?>("feregistro")
                        .HasColumnType("DATETIME");

                    b.HasKey("nmmaestro");

                    b.ToTable("TblMaestra", "Maestra");
                });

            modelBuilder.Entity("ERP.Domain.Entities.PacienteEntities.Paciente", b =>
                {
                    b.Property<int>("nmid")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("cdusuario")
                        .HasColumnType("VARCHAR(150)");

                    b.Property<string>("dsarl")
                        .HasColumnType("VARCHAR(50)");

                    b.Property<string>("dscondicion")
                        .HasColumnType("TEXT");

                    b.Property<string>("dseps")
                        .HasColumnType("VARCHAR(50)");

                    b.Property<DateTime?>("febaja")
                        .HasColumnType("DATETIME");

                    b.Property<DateTime?>("feregistro")
                        .HasColumnType("DATETIME");

                    b.Property<int>("nmid_medicotra")
                        .HasColumnType("int");

                    b.Property<int>("nmid_persona")
                        .HasColumnType("int");

                    b.HasKey("nmid");

                    b.HasIndex("nmid_medicotra");

                    b.HasIndex("nmid_persona")
                        .IsUnique();

                    b.ToTable("TblPaciente", "General");
                });

            modelBuilder.Entity("ERP.Domain.Entities.PersonaEntities.Persona", b =>
                {
                    b.Property<int>("nmid")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("cddocumento")
                        .IsRequired()
                        .HasColumnType("VARCHAR(20)");

                    b.Property<string>("cdgenero")
                        .HasColumnType("VARCHAR(10)");

                    b.Property<string>("cdtelefono_movil")
                        .HasColumnType("VARCHAR(20)");

                    b.Property<string>("cdtelfono_fijo")
                        .HasColumnType("VARCHAR(20)");

                    b.Property<string>("cdtipo")
                        .HasColumnType("VARCHAR(10)");

                    b.Property<string>("cdusuario")
                        .HasColumnType("VARCHAR(150)");

                    b.Property<string>("dsapellidos")
                        .HasColumnType("VARCHAR(60)");

                    b.Property<string>("dsdireccion")
                        .HasColumnType("VARCHAR(200)");

                    b.Property<string>("dsemail")
                        .HasColumnType("VARCHAR(200)");

                    b.Property<string>("dsnombres")
                        .IsRequired()
                        .HasColumnType("VARCHAR(60)");

                    b.Property<string>("dsphoto")
                        .HasColumnType("VARCHAR(500)");

                    b.Property<DateTime?>("febaja")
                        .HasColumnType("DATETIME");

                    b.Property<DateTime>("fenacimiento")
                        .HasColumnType("DATE");

                    b.Property<DateTime?>("feregistro")
                        .HasColumnType("DATETIME");

                    b.HasKey("nmid");

                    b.HasIndex("cddocumento")
                        .IsUnique();

                    b.ToTable("TblPersona", "General");
                });

            modelBuilder.Entity("ERP.Domain.Entities.Maestras.DataMaestra", b =>
                {
                    b.HasOne("ERP.Domain.Entities.Maestras.Maestra", "Maestra")
                        .WithMany("DataMaestra")
                        .HasForeignKey("Maestranmmaestro")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Maestra");
                });

            modelBuilder.Entity("ERP.Domain.Entities.PacienteEntities.Paciente", b =>
                {
                    b.HasOne("ERP.Domain.Entities.PersonaEntities.Persona", "nmidMedicotra")
                        .WithMany("nmid_medicotra")
                        .HasForeignKey("nmid_medicotra")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("ERP.Domain.Entities.PersonaEntities.Persona", "nmidPersona")
                        .WithMany("nmid_persona")
                        .HasForeignKey("nmid_persona")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("nmidMedicotra");

                    b.Navigation("nmidPersona");
                });

            modelBuilder.Entity("ERP.Domain.Entities.Maestras.Maestra", b =>
                {
                    b.Navigation("DataMaestra");
                });

            modelBuilder.Entity("ERP.Domain.Entities.PersonaEntities.Persona", b =>
                {
                    b.Navigation("nmid_medicotra");

                    b.Navigation("nmid_persona");
                });
#pragma warning restore 612, 618
        }
    }
}
